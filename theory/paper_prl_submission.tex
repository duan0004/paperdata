...
cosmological rows.  The $0.681$-nat gap between the leading
SIGW-Gaussian row and the best curved-SMBHB control is comparable to the
...
relative to the SMBHB baseline: $\mathcal{B}=57\pm3$ for
scalar-induced gravitational waves with a Gaussian primordial spectrum
(SIGW-Gaussian), $\mathcal{B}=46\pm4$ for cosmic superstrings, and
$\mathcal{B}=18\pm1$ for a bubble-nucleation phase transition, placing
the leading alternatives in the strong-but-not-decisive evidence range
...
remaining Panel-A bars and all Panel-B rows use the PTArcade BHB-prior
baseline.  Panel B compares leading cosmological templates (blue) with
...
five-configuration means for the tested curved-SMBHB representatives.}
\begin{ruledtabular}
\begin{tabular}{@{}lcc@{}}
...
Ecc.-curved SMBHB & $3.839\pm0.043$ & tested repr. \\
Broken-PL SMBHB & $3.656\pm0.154$ & tested repr. \\
Env. SMBHB & $3.453\pm0.062$ & tested repr. \\
\end{tabular}
\end{ruledtabular}
\end{table}
...
respectively.  The best curved-SMBHB row lies only $0.681$ nat below
...
within $0.12$ nat for the leading cosmological rows and within
...
boundary systematic.  Cumulative-bin evidence shows that all rows except
SIGW-Gaussian are already within $0.5$ nat of their 14-bin values by the
lowest eight Fourier bins; SIGW-Gaussian requires ten bins.
...
{10.5281/zenodo.10495907}.  The code-release URL is
\texttt{https://github.com/duan0004/paperdata}.  Local checksums and reproduction
commands are recorded in \texttt{REPRODUCIBILITY.md}.
