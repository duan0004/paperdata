...
# ── 配置 ─────────────────────────────────────────────────────────────
N_STEPS          = 2_000_000      # 初始目标，可 resume 继续
N_GWBFREQS       = 14
...
# 初值中心（官方 14f-PL HD 后验均值，arXiv:2306.16213 / .core file）
LOG10A_INIT      = -14.20
GAMMA_INIT       = 3.25
INIT_JITTER      = 0.5
...
CHAIN_DIR   = os.path.join(OUT_DIR, "chains_production_HD")
...
def watcher():
    ...
    while not _stop.is_set():
        ...
        try:
            ...
            sp = os.path.join(OUT_DIR, f"T2_summary_prodHD_{tag}.json")
...
fp = os.path.join(OUT_DIR, "T2_summary_prodHD_final.json")
