...
# 初值中心：第三条链（"hot"），在 (log10_A, γ) 平面上与 warm + cold
# 都相距 >5σ；方向选"高振幅+浅谱"对角线，独立检验收敛
# warm posterior: log10_A=-14.20±0.12, γ=3.25±0.32
# cold init:      log10_A=-15.50, γ=5.00（左上远）
# hot init:       log10_A=-13.00, γ=1.50（右下远）
LOG10A_INIT      = -13.00         # 比 warm 后验高 ~10σ
GAMMA_INIT       = 1.50           # 比 warm 后验低 ~5σ
...
CHAIN_DIR   = os.path.join(OUT_DIR, "chains_production_HD_hot")
...
def watcher():
    ...
    while not _stop.is_set():
        ...
        try:
            ...
            sp = os.path.join(OUT_DIR, f"T2_summary_prodHD_hot_{tag}.json")
...
fp = os.path.join(OUT_DIR, "T2_summary_prodHD_hot_final.json")
